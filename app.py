...
def make_main_activity_startapp(pkg, startapp_id, content_type, content_value,
                                 has_banner, has_interstitial, auto_show):
    if content_type == "url":
        load_line = f'webView.loadUrl("{content_value}");'
    else:
        esc = (content_value.replace('\\','\\\\').replace('"','\\"')
...
<LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <Button android:id="@+id/btnInterstitial"
        android:layout_width="match_parent" android:layout_height="wrap_content"
        android:text="📺 Show Ad" android:textColor="#FFFFFF" android:backgroundTint="#FF6B35"/>
</LinearLayout>
"""
...
def build():
    ...
    # Custom ad snippets for HTML content
    if network == "unity":
        r_btn = '<a href="ads://show_rewarded" style="display:inline-block;padding:12px 20px;background:#4CAF50;color:#fff;border-radius:8px;text-decoration:none;font-weight:bold;">🎁 Watch Ad for Reward</a>'
        i_btn = '<a href="ads://show_interstitial" style="display:inline-block;padding:12px 20px;background:#2196F3;color:#fff;border-radius:8px;text-decoration:none;font-weight:bold;">📺 Show Ad</a>'
        js_r  = "window.NativeAds.showRewarded()"
        js_i  = "window.NativeAds.showInterstitial()"
    else:
        r_btn = ""
        i_btn = '<a href="ads://show_interstitial" style="display:inline-block;padding:12px 20px;background:#FF6B35;color:#fff;border-radius:8px;text-decoration:none;font-weight:bold;">📺 Show Ad</a>'
        js_r  = "window.NativeAds.showInterstitial()"
        js_i  = "window.NativeAds.showInterstitial()"
